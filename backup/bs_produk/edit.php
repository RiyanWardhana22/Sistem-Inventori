...
$id = $_GET['id'] ?? null;
if (!$id) redirect(BASE_URL . 'pages/stok_bs/');

$stmt = $pdo->prepare("SELECT * FROM stok_bs WHERE id_bs = ?");
$stmt->execute([$id]);
$bs = $stmt->fetch();
?>
                        ...
                        <form action="proses.php" method="POST">
                                    ...
                                    <div class="row">
                                                <div class="col-md-4 mb-3">
                                                            ...
                                                            <input type="date" class="form-control" id="tanggal_bs" name="tanggal_bs" value="<?php echo $bs['tanggal_bs']; ?>" required>
                                                </div>
                                                <div class="col-md-8 mb-3">
                                                            ...
                                                            <select class="form-select" id="kode_produk" name="kode_produk" required>
                                                                        ...
                                                                        while ($produk = $queryProduk->fetch(PDO::FETCH_ASSOC)) {
                                                                                    $selected = ($produk['kode_produk'] == $bs['kode_produk']) ? 'selected' : '';
                                                ...
                                                <div class="col-md-4 mb-3">
                                                            ...
                                                            <input type="number" class="form-control" id="jumlah" name="jumlah" min="1" value="<?php echo $bs['jumlah']; ?>" required>
                                                </div>
                                                <div class="col-md-8 mb-3">
                                                            <label for="keterangan" class="form-label">Keterangan (Opsional)</label>
                                                            <input type="text" class="form-control" id="keterangan" name="keterangan" value="<?php echo htmlspecialchars($bs['keterangan']); ?>">
                                                </div>
                                    </div>
