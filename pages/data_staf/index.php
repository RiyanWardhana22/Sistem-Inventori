...
$count_stmt = $pdo->prepare("SELECT COUNT(*) FROM pengguna WHERE nama_lengkap LIKE ? OR username LIKE ?");
$count_stmt->execute([$search_query, $search_query]);
$total_records = $count_stmt->fetchColumn();
$total_pages = ceil($total_records / $records_per_page);

$stmt = $pdo->prepare("SELECT * FROM pengguna WHERE nama_lengkap LIKE ? OR username LIKE ? ORDER BY nama_lengkap ASC LIMIT ? OFFSET ?");
            ...
            <div class="card-header">
                        ...
                        <form method="GET" action="">
                                    <div class="input-group">
                                                <input type="text" class="form-control" name="search" placeholder="Cari Pengguna..." value="<?php echo htmlspecialchars($search_term); ?>">
                                                            ...
                                                            ?>
                                                                                    <tr>
                                                                                                ...
                                                                                                <td class="d-flex justify-content-center gap-2">
                                                                                                            <a href="edit.php?id=<?php echo $row['id_pengguna']; ?>" class="btn btn-sm btn-outline-warning"><i class="fa-solid fa-pencil"></i></a>
                                                                                                            <?php if ($_SESSION['user_id'] != $row['id_pengguna']): ?>
                                                                                                                        <a href="proses.php?action=hapus&id=<?php echo $row['id_pengguna']; ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Apakah Anda yakin?');"><i class="fa-solid fa-trash"></i></a>
                                                                                                            <?php endif; ?>
                                                                                                </td>
                                                                                    </tr>
                                                            <?php
                                                                        ...
                                                                        echo "<tr><td colspan='5' class='text-center py-4'>Data staf tidak ditemukan.</td></tr>";
                                                            }
                                                            ?>
                                                </tbody>
                                    </table>
