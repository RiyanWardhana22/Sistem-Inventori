...
$spreadsheet->getProperties()
    ...
    ->setTitle('Data Barang Rusak (BS) Bahan')
    ->setSubject('Export Data Barang Rusak Bahan');
...
foreach ($bs_data as $index => $data) {
    $rowData = [
        ...
        $data['tanggal_bs'],
        $data['nama_bahan'],
        is_numeric($data['jumlah_bahan']) ? $data['jumlah_bahan'] : 0,
        $data['satuan_jumlah'] ?? '-',
        $data['keterangan'] ?? '-'
    ];
...
$sheet->getStyle('D2:D'.$rowNumber)  // Format angka (jumlah)
    ->getNumberFormat()
    ->setFormatCode('#,##0');
...
header('Content-Disposition: attachment;filename="data_bs_bahan_'.date('Y-m-d,H.i.s').'.xlsx"');
